...
public class KurvendisskusionController {
	 ...
	 */
	private volatile boolean l‰uft;
	/**
	 * Referenz zu der Funktion aus {@link #derHaupFensterController}.
	 */
	 ...
	 */
	private HauptFensterController derHaupFensterController;
	...
	private void initialize() {
		...
		diePunkteAnzeigenCheckBox.selectedProperty().addListener((p,o,n)->derHaupFensterController.aendereGroeﬂe());
		setzeAufStandard();
	...
	public void berechneAlle() {
		if (!l‰uft) {
			...
			dieRunnable= new Runnable() {
				...
				public void run() {
					setzeLaeuft(true);
					Runnable r = ()->derHaupFensterController.gibUebersichtController().zuHauptFunktionsButtonsAktiv(false);
					Platform.runLater(r);
					...
					setzeLaeuft(false);
					r = ()->derHaupFensterController.gibUebersichtController().zuHauptFunktionsButtonsAktiv(true);
					Platform.runLater(r);
	...
	private boolean ueberpruefeInterrupt() {
		...
		if(interrupted) {
			...
			Runnable r = ()->derHaupFensterController.gibUebersichtController().zuHauptFunktionsButtonsAktiv(true);
			Platform.runLater(r);
		}
		return interrupted;
	}
	...
	private void aktualisiereZeichnung() {
		derHaupFensterController.aendereGroeﬂe();
	}
	/**
	...
	/**
	 * Setter f&uuml;r {@link #l‰uft}. L‰uft gibt an ob noch Berechnungen in {@link #derRechenThread} laufen.
	 * @param l‰uft gibt an ob {@link #derRechenThread} noch rechnet.
	 */
	private void setzeLaeuft(boolean l‰uft) {
		this.l‰uft=l‰uft;
		this.derHaupFensterController.gibEingabeController().setzeOkButtonDisabled(l‰uft);
		this.derStopButton.setDisable(!l‰uft);
	}
	...
	private void berechneAuswahl() {
		...
		if (!l‰uft) {
			leseIntervallEin();
			derHaupFensterController.gibUebersichtController().zuHauptFunktionsButtonsAktiv(false);
			dieRunnable=new Runnable() {
	...
	/**
	 * Setter f&uuml; {@link #derHaupFensterController}
	 * @param derHaupFensterController der neue {@link HauptFensterController}
	 */
	public void setzeHauptFensterController(HauptFensterController derHaupFensterController) {
		this.derHaupFensterController = derHaupFensterController;
	}
	/**
	 * Getter f&uuml; {@link #dieNullstellen}
	 * @return {@link #dieNullstellen}
