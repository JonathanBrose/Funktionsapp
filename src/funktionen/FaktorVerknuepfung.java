 ...
 * Die Form sieht wiefolgt aus: <code>f(x) = u(x) * v(x) * ... </code> <br>
 * Dieses {@link Funktionsteil} gibt bei {@link #gibStammfunktion()} null
 * zur&uuml;ck.
 * 
...
public class FaktorVerknuepfung extends Verknuepfung {
	 ...
	 *            {@link ArrayList} mit den Funtionsteilen die alle miteinander
	 ...
	 * Die Form sieht wiefolgt aus: <code>f(x) = u(x) * v(x)</code>
	 * 
	 * @param dasFunktionsteilArray
	 *            Array mit den Funtionsteilen die alle miteinander multipliziert
	 *            werden
	 */
	...
	public Funktionsteil gibAbleitung() {
		...
		if (dieFunktionsTeile.size() > 2) {
			ArrayList<Funktionsteil> dieLinkenFunktionsteile = new ArrayList<Funktionsteil>(),
					dieRechtenFunktionsteile = new ArrayList<Funktionsteil>();
			...
			Funktionsteil dasLinkeFunktionsteil = (dieLinkenFunktionsteile.size() == 1 ? dieLinkenFunktionsteile.get(0)
					: new FaktorVerknuepfung(dieLinkenFunktionsteile));
			dieAbleitung = new AddVerknuepfung(
					new FaktorVerknuepfung(dasLinkeFunktionsteil, dasRechteFunktionsteil.gibAbleitung()),
					new FaktorVerknuepfung(dasLinkeFunktionsteil.gibAbleitung(), dasRechteFunktionsteil));
			...
			dieAbleitung = new AddVerknuepfung(new FaktorVerknuepfung(f, u.gibAbleitung()),
					new FaktorVerknuepfung(f.gibAbleitung(), u));
		}
	...
	public String gibString(int i, String variablenName) {
		String s = "";
		ArrayList<Funktionsteil> dieBruchFunktionen = new ArrayList<Funktionsteil>();
		ArrayList<Funktionsteil> dieBruchKombiFunktionen = new ArrayList<Funktionsteil>();
		for (Funktionsteil dasFunktionsteil : dieFunktionsTeile) {
			if (dasFunktionsteil instanceof PotenzFunktion) {
				if (((PotenzFunktion) dasFunktionsteil).gibP() < 0)
					dieBruchFunktionen.add(dasFunktionsteil);
			} else {
				dieBruchKombiFunktionen.add(dasFunktionsteil);
			}
		}
		dieFunktionsTeile.removeAll(dieBruchFunktionen);
		for (Funktionsteil dasBruchFunktionsteil : dieBruchFunktionen) {
			boolean f = false;
			for (Funktionsteil dasFunktionsteil : dieBruchKombiFunktionen) {
				dieFunktionsTeile.remove(dasFunktionsteil);
				f=true;
				String sA[] = ((PotenzFunktion) dasBruchFunktionsteil).getFractionString(i, variablenName);
				s+=sA[0]+dasFunktionsteil.gibString(i, variablenName)+sA[1]+" * ";
				break;
			}
			if(!f) {
				dieFunktionsTeile.add(dasBruchFunktionsteil);
			}
		}
		...
		if (dasFunktionsteil1 instanceof FaktorVerknuepfung) {
			...
			for (Funktionsteil dasFunktionsteil : ((FaktorVerknuepfung) dasFunktionsteil1).gibFunktionsteile()) {
				...
				for (Funktionsteil dasInnereFunktionsteil : dieFunktionsTeile) {
					if (dasFunktionsteil.entspricht(dasInnereFunktionsteil)
							&& dieFunktionsteile.contains(dasInnereFunktionsteil)) {
						entspricht = true;
	...
	/**
	 * Multipliziert alle Vorfaktoren a miteinander und setzt diese Summe dann als
	 * Vorfaktor des ersten {@link Funktionsteil}s<br>
	 * 
	 * 
	 * <b>Beispiel:</b><br>
	 * <ul>
	...
	public void fuegeHinzu(Funktionsteil dasFunktionsteil) {
		...
		if (this.enthaeltFaktorVerknuepfung())
			fasseZusammen();
	}
	 ...
	 * 
	 * @param dieKlasse
	 *            Objekt einer Klasse von der Elemente zur&uuml;ckgegeben werden
	 *            sollen
	 * 
	...
	/**
	 * Verschiebt, falls sich {@link FaktorVerknuepfung}en in der Liste dieser
	 * Instanz befinden, alle Inhalte dieser FaktorVerknuepfung in die Hauptliste
	 * <br>
	 * <br>
	 * 
	 * <b> Beispiel:</b>
	 * <ul>
	 * <li>FaktorVerknuepfung.funktionsTeile[{@link FaktorVerknuepfung}[{@link Sin}
	 * , {@link PotenzFunktion}], {@link ExponentialFunktion}]</li>
	 * </ul>
	 * <ul>
	 * <li>toString:
	 * <code>"(Sin * PotenzFunktion) * ExponentialFunktion"</code></li>
	 * </ul>
	 * <b>ergibt:</b>
	 * <ul>
	 * <li>FaktorVerknuepfung.funktionsTeile[{@link Sin}, {@link PotenzFunktion},
	 * {@link ExponentialFunktion}]</li>
	 * </ul>
	 * <ul>
	 * <li>toString:<code> "Sin * PotenzFunktion * ExponentialFunktion"</code></li>
	 * </ul>
	 */
	private void fasseZusammen() {
		...
		for (Funktionsteil dasFunktionsteil : dieFunktionsTeile) {
			if (dasFunktionsteil instanceof FaktorVerknuepfung) {
				for (Funktionsteil dasInnereFunktionsteil : ((FaktorVerknuepfung) dasFunktionsteil)
						.gibFunktionsteile()) {
