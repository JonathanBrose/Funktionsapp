...
/**
 * Verwaltet {@link #dasCanvas} und zeichnet mithilfe von
 * {@link #derGrafikKontext} darauf.
 * 
 * @author Jonathan
 *
 */
public class Plotter {
	...
	/**
	 * In wie viele Einheiten die X-Achse pro Seite bei 1.0 {@link #skalierung}
	 * unterteilt ist.
	 */
	private static final double EINHEITEN_PRO_SEITE_X = 10;
	/**
	 * Der maximale zoom, bzw. der kleinste Wert den {@link #skalierung} annehmen
	 * darf.
	 */
	 ...
	 * 
	 * @param dasCanvas
	 *            das zu verwaltende {@link Canvas}.
	 */
	 ...
	 * 
	 * @param dieFunktion
	 *            die Funktion die gezeichnet werden soll.
	 * @param dieFarbe
	 *            Farbe in der die Funktion gezeichnet werden soll.
	 */
	public void plotteFunktion(Funktion dieFunktion, Color dieFarbe) {
		if (dieFunktion == null || dieFunktion.entsprichtXAchse())
			return;
		...
		derGrafikKontext.setLineCap(StrokeLineCap.ROUND);
		...
		for (int i = 0; i <= dasCanvas.getWidth(); i++) {
			...
			if (i == 0 || warNaN) {
				warNaN = false;
				if (sollUebersprungenWerden(derFunktionswert, x, x2, dieFunktion, abstand)) {
					warNaN = true;
					continue;
				} else {
					derGrafikKontext.moveTo(i, (-derFunktionswert - dY) * abstand + yVerschiebung);
					derGrafikKontext.beginPath();
					derGrafikKontext.lineTo(i, (-derFunktionswert - dY) * abstand + yVerschiebung);
				}
			} else if (sollUebersprungenWerden(derFunktionswert, x, x2, dieFunktion, abstand)) {
				warNaN = true;
		...
		if (warNaN) {
			derGrafikKontext.beginPath();
			derGrafikKontext.moveTo(dasCanvas.getWidth() - 1, 1);
			derGrafikKontext.lineTo(dasCanvas.getWidth(), 1);
		}
		derGrafikKontext.stroke();
		derGrafikKontext.closePath();
	...
	/**
	 * &Uuml;berpr&uuml;ft ob die n&auml;chste Linie gezeichnet, oder
	 * &uuml;bersprungen werden soll.
	 * 
	 * @param funktionsWert
	 *            aktueller Funktionswert.
	 * @param x
	 *            x-Koordinate im Koordinatensystem.
	 * @param dieFunktion
	 *            die {@link Funktion}, die gezeichnet werden soll.
	 * @param x2
	 *            x-Koordinate, die beim n&auml;chsten Schleifendurchlauf kommen
	 *            wird.
	...
	private boolean sollUebersprungenWerden(Double funktionsWert, double x, double x2, Funktion dieFunktion,
			double abstand) {
		if (!UeberpruefeDoubleGueltig(funktionsWert))
			return true;

		if ((-funktionsWert - dY) * abstand < -dasCanvas.getHeight() / 2
				|| (-funktionsWert - dY) * abstand > dasCanvas.getHeight() / 2) {

		}
		if (dieFunktion.berechneFunktionswert(x) > 0 && dieFunktion.berechneFunktionswert(x2) < 0) {
			if (!UeberpruefeDoubleGueltig(dieFunktion.berechneFunktionswert(runde(x, 1))))
				return true;
			if (!UeberpruefeDoubleGueltig(dieFunktion.berechneFunktionswert(runde(x + runde(x - x2, 1), 1))))
				return true;
			if (!UeberpruefeDoubleGueltig(dieFunktion.berechneFunktionswert(runde(x2, 1))))
				return true;
		}
		if (dieFunktion.berechneFunktionswert(x) < 0 && dieFunktion.berechneFunktionswert(x2) > 0) {
			if (!UeberpruefeDoubleGueltig(dieFunktion.berechneFunktionswert(runde(x, 1))))
	...
	/**
	 * Zeichnet die von dem {@link KurvendisskusionController} errechneten Punkte in
	 * den Graphen ein.
	 * 
	 * @param derKurvendisskusionController
	 *            der {@link KurvendisskusionController} der die zu zeichnenden
	 *            Punkte hat.
	 * @param dieFarbe
	 *            Farbe in der die {@link Punkt}e gezeichnet werden sollen.
	 */
	 ...
	 * 
	 * @param linienDicke
	 *            Dicke der Linien in pixeln.
	 * @param dieFarbe
	 *            {@link Color}, Farbe der Achsen.
	 * @param abstand
	 *            Abstand zwischen einer Koordinatensystem Einheit in pixeln.
	 */
	 ...
	 * 
	 * @param linienDicke
	 *            die Dicke der Linien des Textes.
	 * @param dieFarbe
	 *            die Farbe in der gezeichnet wird.
	 * @param abstand
	 *            ein Koordinateneinheit in pixeln.
	 * @param kommaUnterteilungen
	 *            wie oft die Beschriftung in Kommazahlen aufgeteilt werden soll.
	 * @param schriftGroeﬂe
	 *            die Schriftgr&ouml;&szlig;e
	 * @param maxZahlenBreite
	 *            die maximale Breite die ein einzelner String haben darf.
	 */
	 ...
	 * 
	 * @param abstandTeilung
	 *            wie oft das Gitter geteilt werden soll pro Einheit.
	 * @param linienBreite
	 *            Breite der Linien.
	 * @param dieFarbe
	 *            Farbe der Linien.
	 ...
	 * 
	 * @param diePunktListe
	 *            die Liste mit den Punkten, die gezeichnet werden sollen.
	 * @param dieBenutztenPunkte
	 *            Liste mit den Punkten, die schon gezeichnet wurden.
	 * @param dieFarbe
	 *            Farbe in der die Punkte gezeichnet werden sollen.
	 */
	private void plottePunktListe(ArrayList<Punkt> diePunktListe, ArrayList<Punkt> dieBenutztenPunkte, Color dieFarbe) {
		...
		for (Punkt derPunkt : diePunktListe) {
			...
			int c = uberpruefePunktBenutzt(dieBenutztenPunkte, derPunkt);
			if (c > 0) {
				yPos -= schriftGroeﬂe * c;
			} else {
	...
	/**
	 * &Uuml;berpr&uuml;ft ob in der ArrayList ein Punkt mit den gleichen Werten,
	 * wie derPunkt ist.
	 * 
	 * @param dieBenutztenPunkte
	 *            Liste mit den bereits geplotteten Punkten.
	 * @param derPunkt
	 *            {@link Punkt} der auf Gleichheit &uuml;berpr&uuml;ft werden soll.
	 * @return true wenn ein gleicher Punkt enthalten ist, ansonsten false;
	 */
	private int uberpruefePunktBenutzt(ArrayList<Punkt> dieBenutztenPunkte, Punkt derPunkt) {
		int c = 0;
		for (Punkt derBenutztePunkt : dieBenutztenPunkte) {
			if (derBenutztePunkt.entspricht(derPunkt))
				c++;
		}
		return c;
	}
	...
	/**
	 * Hilfsmethode die XVerschiebung f&uuml;r die Beschriftung der X-Achse
	 * Begrenzt.
	 * 
	 * @param verschiebung
	...
	/**
	 * Hilfsmethode die XVerschiebung f&uuml;r die Beschriftung der Y-Achse
	 * Begrenzt.
	 * 
	 * @param verschiebung
	 *            die eigentliche Verschiebung.
	...
	/**
	 * Berechnet aus einem double den passenden String f&uuml;r die
	 * Achsenbeschriftung.
	 * 
	 * @param c
	 *            der double, der umgewandelt werden soll.
	...
	/**
	 * Berechnet einen Multiplikator, durch den die Linien abh&auml;ngig von der
	 * {@link #skalierung} dicker, bzw. d&uuml;nner werden.
	 * 
	...
	/**
	 * Setter f&uuml;r {@link #skalierung}. Ist begrenzt durch
	 * {@link #MINIMALE_SKALIERUNG}.
	 * 
	 * @param skalierung
	 *            neuer Wert f&uuml;r {@link #skalierung}, kann nicht kleiner werden
	 *            als {@value #MINIMALE_SKALIERUNG}.
	 */
	 ...
	 * 
	 * @param addition
	 *            der Wert der auf {@link #dX} aufaddiert wird.
	 */
	 ...
	 * 
	 * @param addition
	 *            der Wert der auf {@link #dY} aufaddiert wird.
	 */
	...
	/**
	 * Addiert den parameter zu {@link #skalierung}. Der Wert wird allerdings in
	 * Abh&auml;ngigkeit von {@link #skalierung} herunterskaliert, um die
	 * Ver&auml;nderung nicht zu gro&szlig; zu machen.
	 * 
	 * @param addition
	 *            der Wert der auf {@link #skalierung} aufaddiert wird.
	 */
